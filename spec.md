...
- JWST telescope swarm at L3 (Goal 11: relocated from its original L2 placement, reshaped into a concave dish, then given its own dedicated relay network): 1000 individually selectable `space-telescope` devices arranged on a spherical-cap (dish) surface at the Sun-Earth L3 point (far side of the sun from Earth), all facing outward into deep space via one whole-group quaternion rotation rather than per-instance orientation. Rendered via `THREE.InstancedMesh` for one draw call, with per-instance click selection via `instanceId` and on-demand (not pre-stored) info generation. Powered by a private 4-hop chain (L3 Relay Station + 3 L3 Feed Relays) positioned behind the dish and rigidly following its position/orientation, tapping the shared power relay mesh only at its far end rather than every hop being shared with other consumers. The first concrete, if smaller-scale, instance of Goal 1's "millions of telescopes acting as one" vision. Still open: the swarm doesn't yet act as a combined instrument in any functional sense (no data-fusion/aggregate behavior, just 1000 independently-clickable points sharing one power tap and one facing direction) — that's the natural next step if this goal is revisited.
